...
def balanced(m):
    ...
    def better_weight(a):
        if is_weight(a):
            ...
            return better_weight(end(a))
        elif is_mobile(a):
            return better_weight(left(a))+better_weight(right(a))
    def helper(x):
        if is_mobile(x):
            if length(left(x))*better_weight(left(x))!=length(right(x))*better_weight(right(x)):
                return False
        if is_mobile(end(left(x))):
            if helper(end(left(x)))==False:
                return False
        if is_mobile(end(right(x))):
            if helper(end(right(x)))==False:
                return False
    return (None==helper(m))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,password):
        nonlocal balance, wrongs,wrong_count
        if wrong_count==3:
            return "Your account is locked. Attempts: "+str(wrongs)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(x)==str:
        ...
        def withdraw2(balance, password):
