...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    if sqrt((get_lon(city1) - lon)**2 + (get_lat(city1) - lat)**2)>sqrt((lon - get_lon(city2))**2 + (lat - get_lat(city2))**2):
    ...
    1
      2
        3
          ...
          2
    """
    make_tree=[tree(val) for val in vals]
    new_tree=[label(t)]
    if is_leaf(t):
        return t+make_tree
    elif not is_leaf(t):
        return new_tree+[sprout_leaves(b,vals) for b in branches(t)]
