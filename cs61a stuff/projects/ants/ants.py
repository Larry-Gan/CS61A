...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.scared_counter>1:
            self.scared_immune=True
        if self.direction and not self.scared_immune:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dist=0
        while place_checker!=beehive:
            if random_or_none(place_checker.bees) and self.min_range<=dist and self.max_range>=dist:
                return random_or_none(place_checker.bees)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        if self.place.bees:
            ...
            target_bee.reduce_armor(target_bee.armor)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self, armor)
        QueenAnt.list_of_queens.append(self)
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if QueenAnt.list_of_queens[0] is not self:
            Insect.reduce_armor(self,self.armor)
        else:
            ant_finder=self.place.exit
            while ant_finder!=None:
                if ant_finder.ant and ant_finder.ant.not_buffed:
                    if isinstance(ant_finder.ant,BodyguardAnt):
                        if ant_finder.ant.contained_ant and ant_finder.ant.contained_ant.not_buffed:
                            ant_finder.ant.contained_ant.damage=ant_finder.ant.contained_ant.damage*2
                            ant_finder.ant.contained_ant.not_buffed=False
                    ant_finder.ant.damage=ant_finder.ant.damage*2
                    ant_finder.ant.not_buffed=False
                if ant_finder.ant and ant_finder.ant.not_buffed==False:
                    if isinstance(ant_finder.ant,BodyguardAnt):
                        if ant_finder.ant.contained_ant and ant_finder.ant.contained_ant.not_buffed:
                            ant_finder.ant.contained_ant.damage=ant_finder.ant.contained_ant.damage*2
                            ant_finder.ant.contained_ant.not_buffed=False
                ant_finder=ant_finder.exit
    ...
    def reduce_armor(self, amount):
        ...
        if QueenAnt.list_of_queens[0] is self:
            if self.armor<=0:
                return bees_win()
        # END Problem 13
