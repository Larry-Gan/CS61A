...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        def helper(self, formals, vals):
            if formals == nil and vals != nil:
                raise SchemeError('formals is nil')
            if formals != nil and vals == nil:
                raise SchemeError('vals is nil')
            if formals == nil or vals == nil:
                #child_frame.define(formals.first,vals.first)
                return child_frame
            else:
                child_frame.define(formals.first,vals.first)
            return helper(self, formals.rest, vals.rest)
        return helper(self, formals, vals)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        expressions_without_name=Pair(target.rest,expressions.rest)
        env.define(target.first, do_lambda_form(expressions_without_name, env))
        return target.first
        # END PROBLEM 9
    else:
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.rest is nil:
                return test
            elif clause.rest.__len__ is not 0:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    def make_var(bindings):
        if bindings is nil:
            return nil
        return Pair(bindings.first.first,make_var(bindings.rest))
    def make_val(bindings):
        if bindings is nil:
            return nil
        check_form(bindings.first,0,2)
        return Pair(scheme_eval(bindings.first.rest.first,env),make_val(bindings.rest))
    variables=make_var(bindings)
    values=make_val(bindings)
